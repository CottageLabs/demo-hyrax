...
module Hyrax
  ...
  module FilterSuppressedWithRoles
    ...
    def only_active_works(solr_parameters)
      ...
      return if anonymous_reviewer?(current_work: current_work)
      super
    end
    ...
    def proxy_depositor?(current_work:)
      current_user and current_work&.proxy_depositor&.include?(current_user.user_key)
    end

    def anonymous_reviewer?(current_work:)
      return true if current_ability.can?(:read, current_work)
    end
  end
end