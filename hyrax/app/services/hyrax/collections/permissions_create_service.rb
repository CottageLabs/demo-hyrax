      ...
      class << self
        ...
        def create_default(collection:, creating_user:, grants: [])
          ...
          access_grants = access_grants_attributes(collection_type: collection_type, creating_user: creating_user,
                                                   grants: grants, collection: collection)
          template = Hyrax::PermissionTemplate.find_or_create_by!(source_id: collection.id.to_s)

          access_grants.each do |grant|
            permission_template = Hyrax::PermissionTemplateAccess.find_or_initialize_by(permission_template_id: template.id.to_s,
                                                                                        agent_type: grant[:agent_type],
                                                                                        agent_id: grant[:agent_id],
                                                                                        access: grant[:access])

            permission_template.is_system_generated = grant[:is_system_generated] if grant[:is_system_generated]
        ...
        def access_grants_attributes(collection_type:, creating_user:, grants:, collection:)
          agents = [
            { agent_type: 'group', agent_id: admin_group_name, access: Hyrax::PermissionTemplateAccess::MANAGE,
              is_system_generated: true }
          ]

          agents.tap do |attribute_list|
            # Grant manage access to the creating_user if it exists
            if creating_user
              attribute_list << { agent_type: 'user', agent_id: creating_user.user_key,
                                  access: Hyrax::PermissionTemplateAccess::MANAGE, is_system_generated: true }
            end
