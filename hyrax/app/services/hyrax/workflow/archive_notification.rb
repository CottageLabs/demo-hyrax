...
module Hyrax
  module Workflow
    class ArchiveNotification < AbstractNotification
      private

      def subject
        I18n.t('hyrax.notifications.workflow.archived.subject', user: user.user_key)
      end

      def message
        I18n.t('hyrax.notifications.workflow.archived.message',
                title: title,
      ...
      def users_to_notify
        if @entity.proxy_for.class.to_s == 'Dataset'
          [::User.find_by(email: @entity.proxy_for.depositor)] + ::User.includes(:roles).where(roles: { name: "publication_manager" }).to_a
